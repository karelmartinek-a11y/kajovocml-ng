...
UPDATE kcml.generation_job SET lifecycle = CASE lifecycle
  ...
  WHEN 'PLANNING' THEN 'IMPLEMENTING' WHEN 'SUCCEEDED' THEN 'COMPLETED'
  WHEN 'FAILED_FINAL' THEN 'FAILED' WHEN 'CANCELLED_FINAL' THEN 'CANCELLED'
  WHEN 'MANUAL_REVIEW' THEN 'BLOCKED' ELSE current_phase END, state_version = state_version + 1 WHERE lifecycle IN ('INTAKE','DISCOVERY','SPECIFICATION','APPROVAL_REQUIRED','PLANNING','SUCCEEDED','FAILED_FINAL','CANCELLED_FINAL','MANUAL_REVIEW') OR current_phase IN ('INTAKE','DISCOVERY','SPECIFICATION','APPROVAL_REQUIRED','PLANNING','SUCCEEDED','FAILED_FINAL','CANCELLED_FINAL','MANUAL_REVIEW');
