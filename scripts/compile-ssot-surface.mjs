...
const errorsDoc = JSON.parse(await readFile(join(root, 'contracts/registries/errors/errors.json'), 'utf8'));
const errorCodes = errorsDoc.records
  .filter((record) => record.lifecycle === 'ACTIVE')
  .map((record) => record.code)
  .sort();
if (errorCodes.length === 0 || new Set(errorCodes).size !== errorCodes.length || errorCodes.some((code) => !/^[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+$/u.test(code))) throw new Error('ERROR_CODE_GENERATION_INPUT_INVALID');
...
const generatedErrorCodesTs = `// Generated by pnpm contracts:build from contracts/registries/errors/errors.json. Do not edit.\nexport const errorCodes = ${JSON.stringify(errorCodes, null, 2)} as const;\nexport type ErrorCode = (typeof errorCodes)[number];\nexport const errorCodeSet: ReadonlySet<string> = new Set(errorCodes);\n`;
...
const outputs = [
  ...
  [join(root, 'packages/schemas/src/error-codes.generated.ts'), generatedErrorCodesTs],
