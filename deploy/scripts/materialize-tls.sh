...
set -Eeuo pipefail
[[ ${EUID} -eq 0 ]] || exit 77
live=/etc/letsencrypt/live/kajovocml.hcasc.cz
...
grep -Fq 'DNS:kajovocml.hcasc.cz' <<<"${sans}"
grep -Fq 'DNS:*.kajovocml.hcasc.cz' <<<"${sans}"
...
curl --fail --silent --show-error --resolve kajovocml.hcasc.cz:443:127.0.0.1 https://kajovocml.hcasc.cz/health | jq -e '.status=="healthy"' >/dev/null
