...
describe('Runtime Boundary Matrix', () => {
  ...
  it('RUNTIME_GATEWAY_IDENTITY_EVIDENCE: uses native framed anonymous IPC with monotonic sequence', () => {
    const encoded = encodeRuntimeFrame({ frameType: 'REQUEST', flags: 0, sequence: 1, payload: { requestId: 'test' } });
    expect(decodeRuntimeFrameHeader(encoded.subarray(0, 16))).toMatchObject({ frameType: 'REQUEST', payloadLength: encoded.length - 16, sequence: 1 });
