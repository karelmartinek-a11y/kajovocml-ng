...
try {
  if (process.platform !== 'linux') {
    console.log('NOT_EXECUTED_ENVIRONMENTAL: SO_PEERCRED helper requires Linux');
    process.exitCode = 0;
  } else {
    const helperPath = join(root, 'kcml-peercred');
    await promisify(execFile)('cc', ['-O2', '-Wall', '-Wextra', '-Werror', 'deploy/runtime/kcml-peercred.c', '-o', helperPath]);
    process.env.KCML_PEERCRED_HELPER = helperPath;
