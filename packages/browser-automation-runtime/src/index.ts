...
export class ManagedBrowserHost{
  ...
  private async attachOne():Promise<void>{
    ...
    const context=await this.#browser.newContext({acceptDownloads:false,viewport:{width:1440,height:900},locale:'cs-CZ',timezoneId:'Europe/Prague',serviceWorkers:'block'});const page=await context.newPage();
  ...
  private async dispatch(page:Page,action:string,target:Locator|null,payload:Record<string,unknown>):Promise<boolean>{switch(action){case'NAVIGATE':await page.goto(String(payload.url),{waitUntil:'domcontentloaded',timeout:30_000});return true;case'CLICK':if(!target)throw new Error('BROWSER_TARGET_REQUIRED');await target.click({timeout:15_000});return true;case'FILL':if(!target)throw new Error('BROWSER_TARGET_REQUIRED');await target.fill(String(payload.value));return true;case'TYPE':if(!target)throw new Error('BROWSER_TARGET_REQUIRED');await target.pressSequentially(String(payload.value),{delay:Math.min(Number(payload.delayMs??25),250)});return true;case'KEYBOARD':await page.keyboard.press(String(payload.key));return true;case'POINTER':await page.mouse.click(Number(payload.x),Number(payload.y),{button:payload.button==='right'?'right':'left'});return true;case'TOUCH':await page.touchscreen.tap(Number(payload.x),Number(payload.y));return true;case'OBSERVE':return false;default:throw new Error(`BROWSER_ACTION_UNSUPPORTED:${action}`);}}
  ...
  private async heartbeat(status:'READY'|'DRAINING'):Promise<void>{const epoch=await this.pool.query(`SELECT current_epoch FROM kcml.application_deployment_head WHERE singleton_key=1`);await this.pool.query(`INSERT INTO kcml.platform_worker_heartbeat(service_name,instance_id,release_id,source_sha,deployment_epoch,status,details,expires_at)VALUES('kcml-browser-host',$1,$2,$3,$4,$5,$6,clock_timestamp()+interval '30 seconds')ON CONFLICT(service_name,instance_id)DO UPDATE SET status=EXCLUDED.status,details=EXCLUDED.details,observed_at=clock_timestamp(),expires_at=EXCLUDED.expires_at`,[this.#hostId,process.env.KCML_RELEASE_ID??'development',requiredSourceSha(),epoch.rows[0].current_epoch,status,{contexts:this.#contexts.size,runtimeBuildId:this.options.runtimeBuildId}]);}
}
