...
import { captureArtifactDownload, setArtifactInputFiles } from './index.js';
...
const transferArtifactSchema=z.object({handleId:z.string().uuid(),storageReference:z.string().regex(/^artifact:sha256:[a-f0-9]{64}$/u),contentDigest:z.string().regex(/^sha256:[a-f0-9]{64}$/u),sizeBytes:z.number().int().nonnegative(),mimeType:z.string().min(3).max(255),safeName:z.string().min(1).max(255)}).strict();
...
export const browserHostRequestSchema = z.discriminatedUnion('kind', [
  ...
  z.object({ ...requestBase, kind: z.literal('ACTION'), identity: identitySchema, lease: leaseSchema, actionId: z.string().uuid(), actionFence: z.coerce.bigint().positive(), action: z.enum(browserActionNames), locatorAst: z.record(z.string(),z.unknown()).nullable(), payload: z.record(z.string(),z.unknown()), allowedOrigins: z.array(z.string().url()).min(1).max(64), uploadArtifact:transferArtifactSchema.nullable().optional(),downloadId:z.string().uuid().nullable().optional(),downloadArtifactId:z.string().uuid().nullable().optional() }).strict(),
...
export class BrowserHostProtocolServer {
  ...
  private async handle(request:BrowserHostRequest):Promise<Record<string,unknown>> {
    ...
    const locator=request.locatorAst?this.resolveLocator(managed.page,request.locatorAst):null;const dispatched=await this.dispatch(managed,request,locator);
  ...
  private async dispatch(managed:ManagedContext,request:Extract<BrowserHostRequest,{kind:'ACTION'}>,target:Locator|null):Promise<Record<string,unknown>> {
    const {page}=managed,{action,payload}=request;
    switch(action){case'NAVIGATE':await page.goto(String(payload.url),{waitUntil:'domcontentloaded',timeout:30_000});return{mutationTriggerObserved:false,navigationObserved:true};case'CLICK':await target!.click({timeout:15_000});return{mutationTriggerObserved:true};case'FILL':await target!.fill(String(payload.value));return{mutationTriggerObserved:true};case'TYPE':await target!.pressSequentially(String(payload.value),{delay:Math.min(Number(payload.delayMs??25),250)});return{mutationTriggerObserved:true};case'KEYBOARD':await page.keyboard.press(String(payload.key));return{mutationTriggerObserved:true};case'POINTER':await page.mouse.click(Number(payload.x),Number(payload.y));return{mutationTriggerObserved:true};case'TOUCH':await page.touchscreen.tap(Number(payload.x),Number(payload.y));return{mutationTriggerObserved:true};case'UPLOAD':{const artifact=request.uploadArtifact;if(!artifact)throw new Error('BROWSER_UPLOAD_HANDLE_REQUIRED');const fetched=await this.#artifacts.get({sessionId:managed.identity.sessionId,actionId:request.actionId,actionFence:request.actionFence,storageReference:artifact.storageReference,contentDigest:artifact.contentDigest,sizeBytes:artifact.sizeBytes,mimeType:artifact.mimeType});if(!fetched.ok||!fetched.contentBase64)throw new Error(fetched.error?.code??'BROWSER_UPLOAD_ARTIFACT_READ_FAILED');const content=Buffer.from(fetched.contentBase64,'base64');await setArtifactInputFiles(target!,{safeName:artifact.safeName,mimeType:artifact.mimeType,content});return{mutationTriggerObserved:true,uploadConsumedCandidate:{handleId:artifact.handleId,contentDigest:artifact.contentDigest,sizeBytes:content.length}};}case'DOWNLOAD':{if(!request.downloadId||!request.downloadArtifactId)throw new Error('BROWSER_DOWNLOAD_IDENTITY_REQUIRED');const captured=await captureArtifactDownload(page,target!);const contentDigest=`sha256:${createHash('sha256').update(captured.content).digest('hex')}`;const stored=await this.#artifacts.put({sessionId:managed.identity.sessionId,actionId:request.actionId,actionFence:request.actionFence,contentDigest,sizeBytes:captured.content.length,mimeType:captured.mimeType,contentBase64:captured.content.toString('base64')});if(!stored.ok||!stored.artifact)throw new Error(stored.error?.code??'BROWSER_DOWNLOAD_ARTIFACT_PERSIST_FAILED');return{mutationTriggerObserved:true,downloadCandidate:{downloadId:request.downloadId,artifactId:request.downloadArtifactId,safeName:captured.safeName,sourceUrl:captured.sourceUrl,sourceOrigin:currentOrigin(page),...stored.artifact}};}case'DIALOG':case'PERMISSION':case'CHALLENGE':return{mutationTriggerObserved:false,challengeRequired:true,challengeType:action==='CHALLENGE'?String(payload.kind):action,safePrompt:'Owner action is required in the browser',allowedResolutionMethods:action==='CHALLENGE'&&String(payload.kind).toUpperCase().includes('PASSKEY')?['OWNER_DEVICE_WEBAUTHN']:['OWNER_DEVICE']};case'OBSERVE':return{mutationTriggerObserved:false,readOnly:true};}
  }
