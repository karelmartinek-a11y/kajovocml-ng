...
const MANUAL_INVENTORY_QUERIES:readonly {ownerKind:string;table:string;predicate:string;snapshot:string;ownerIdExpr?:string}[]=[
  ...
  {ownerKind:'BROWSER_SESSION',table:'browser_session',predicate:`lifecycle NOT IN ('CLOSED','FAILED_FINAL')`,snapshot:`jsonb_build_object('lifecycle',lifecycle,'stateVersion',state_version,'controlFence',control_fence,'contextGeneration',context_generation)`},
...
export class PlatformRecoveryCoordinator {
  ...
  private async finalize(authority:RecoveryAuthority,workerId:string):Promise<PlatformRecoveryResult>{return inTransaction(this.pool,'SERIALIZABLE',async(client)=>{
    ...
    const blockers=items.filter((item)=>item.blocking).map((item)=>`${item.owner_kind}:${item.owner_id}`);const currentConstraintDigest=await schemaConstraintDigest(client);const schemaStable=Buffer.from(attempt.schema_constraint_digest).equals(currentConstraintDigest);
    const unresolved=[...new Set([...missing,...changed,...blockers,...(!digestStable?['INVENTORY_DIGEST_CHANGED']:[]),...(!schemaStable?['SCHEMA_CONSTRAINT_DIGEST_CHANGED']:[])])].sort();
