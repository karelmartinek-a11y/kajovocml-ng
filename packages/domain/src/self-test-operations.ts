...
function boundedLimit(value:unknown,defaultValue:number):number{if(value===undefined)return defaultValue;if(typeof value!=='number'||!Number.isSafeInteger(value)||value<1||value>500)throw new DomainError('SELF_TEST_LIMIT_INVALID','limit must be an integer between 1 and 500',422,'DO_NOT_RETRY');return value;}
function exactRunId(targetId:string|null,operationName:string):string{if(!targetId||!UUID_PATTERN.test(targetId))throw new DomainError('SELF_TEST_RUN_TARGET_REQUIRED',`${operationName} requires an exact self-test run UUID`,422,'DO_NOT_RETRY');return targetId;}
...
async function listCatalog(pool:DatabasePool,targetId:string|null,args:JsonObject):Promise<unknown>{
  if(targetId!==null)throw new DomainError('SELF_TEST_CATALOG_TARGET_FORBIDDEN','selfTest.catalog.list does not accept a target ID',422,'DO_NOT_RETRY');
  const limit=boundedLimit(args.limit,200);for(const key of ['suite','capability','targetOperation'])if(args[key]!==undefined&&(typeof args[key]!=='string'||String(args[key]).length===0))throw new DomainError('SELF_TEST_CATALOG_FILTER_INVALID',`${key} must be a non-empty string`,422,'DO_NOT_RETRY');
...
async function readEvidence(pool:DatabasePool,targetId:string|null,args:JsonObject):Promise<unknown>{
  ...
  if(evidenceId!==undefined&&(typeof evidenceId!=='string'||!UUID_PATTERN.test(evidenceId)))throw new DomainError('SELF_TEST_EVIDENCE_ID_INVALID','evidenceId must be a UUID',422,'DO_NOT_RETRY');
  if(sequence!==undefined&&(typeof sequence!=='number'||!Number.isSafeInteger(sequence)||sequence<1))throw new DomainError('SELF_TEST_EVIDENCE_SEQUENCE_INVALID','sequence must be a positive safe integer',422,'DO_NOT_RETRY');
  if(evidenceId!==undefined&&sequence!==undefined)throw new DomainError('SELF_TEST_EVIDENCE_SELECTOR_CONFLICT','Use either evidenceId or sequence, not both',422,'DO_NOT_RETRY');
  return consistentRead(pool,async client=>{
    const run=(await client.query(`SELECT id,status,source_sha,release_id,environment_digest,platform_incarnation_id,application_deployment_epoch FROM kcml.self_test_run WHERE id=$1`,[runId])).rows[0];if(!run)throw new DomainError('SELF_TEST_RUN_NOT_FOUND','Self-test run does not exist',404,'DO_NOT_RETRY');
...
export async function executeExactSelfTestQuery(pool:DatabasePool,operationName:string,targetId:string|null,args:JsonObject):Promise<unknown>{if(operationName==='selfTest.catalog.list')return listCatalog(pool,targetId,args);if(operationName==='selfTest.run.status')return runStatus(pool,targetId);if(operationName==='selfTest.evidence.read')return readEvidence(pool,targetId,args);throw new DomainError('SELF_TEST_OPERATION_NOT_EXACT',`Self-test operation ${operationName} has no exact query implementation`,500,'DO_NOT_RETRY');}
